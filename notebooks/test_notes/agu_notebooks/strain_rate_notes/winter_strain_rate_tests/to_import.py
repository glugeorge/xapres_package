...
def generate_plots(station,stacking,vd_window,vd_sum,vd_coarse):
    ...
    # Determine profiles
    profiles = ds_winter.isel(attenuator_setting_pair=1,time=range(10,len(ds_winter.time))).profile_stacked.resample(time=stacking).mean(dim='time').compute()
    ...
    disp_stack = disp_stack.coarsen(profile_range=vd_coarse,boundary='trim').mean()
    err_stack = ds.err.resample(time=vd_sum).apply(error_prop)/convert_to_seconds(vd_sum)*31536000
    err_stack = np.sqrt((err_stack**2).coarsen(profile_range=vd_coarse,boundary='trim').sum())/vd_coarse
